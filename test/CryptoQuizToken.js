...
contract('CryptoQuizToken', function(accounts) {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', function() {
    return CryptoQuizToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'CryptoQuiz Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'CQ', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'CryptoQuiz Token v1.0', 'has the correct standard');
    });
  })
  ...
  it('allocates the initial supply upon deployment', function() {
    return CryptoQuizToken.deployed().then(function(instance) {
      ...
      assert.equal(adminBalance.toNumber(), 500000, 'it allocates the initial supply to the admin account');
      // Another 500,000 is transferred to TokenSale address
    });
  });
