...
contract('CryptoQuizTokenSale', function(accounts) {
  var tokenInstance;
  ...
  var tokenPrice = 1e16; // in unit of wei, == 0.01 eth
  var tokensAvailable = 500000;
  var numberOfTokens;

  it('initializes the contract with the correct values', function() {
    return CryptoQuizTokenSale.deployed().then(function(instance) {
      ...
      return CryptoQuizToken.deployed();
    })
    .then(function(instance) {
      // Make sure TokenSale has initial tokens
      return instance.balanceOf(tokenSaleInstance.address);
    })
    .then(function(balance) {
      // Make sure TokenSale has initial tokens
      assert.equal(balance.toNumber(), 500000, 'should have initial 500,000 tokens');
    })
  });
